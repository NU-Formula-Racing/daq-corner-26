...
uint32_t HAL_CAN_GetTxTimestamp(const CAN_HandleTypeDef* hcan, uint32_t TxMailbox) {
    ...
    if ((state == HAL_CAN_STATE_READY) || (state == HAL_CAN_STATE_LISTENING)) {
        ...
        /* Get timestamp */
        timestamp = (hcan->Instance->sTxMailBox[transmitmailbox].TDTR & CAN_TDT0R_TIME) >>
                    CAN_TDT0R_TIME_Pos;
    }
...
HAL_StatusTypeDef HAL_CAN_GetRxMessage(CAN_HandleTypeDef* hcan, uint32_t RxFifo,
    ...
    if ((state == HAL_CAN_STATE_READY) || (state == HAL_CAN_STATE_LISTENING)) {
        ...
        if (((CAN_RDT0R_DLC & hcan->Instance->sFIFOMailBox[RxFifo].RDTR) >> CAN_RDT0R_DLC_Pos) >=
            8U) {
            /* Truncate DLC to 8 if received field is over range */
            pHeader->DLC = 8U;
        } else {
            pHeader->DLC =
                (CAN_RDT0R_DLC & hcan->Instance->sFIFOMailBox[RxFifo].RDTR) >> CAN_RDT0R_DLC_Pos;
        }
