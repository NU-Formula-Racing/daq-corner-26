...
#define ADC_REG_TRIG_EXT_EDGE_DEFAULT                                                            \
    (ADC_CR2_EXTEN_0) /* Trigger edge set to rising edge (default setting for compatibility with \
                         some ADC on other STM32 families having this setting set by HW default  \
                         value) */
...
#define ADC_INJ_TRIG_EXT_EDGE_DEFAULT                                                             \
    (ADC_CR2_JEXTEN_0) /* Trigger edge set to rising edge (default setting for compatibility with \
                          some ADC on other STM32 families having this setting set by HW default  \
                          value) */
...
#define ADC_CHANNEL_ID_INTERNAL_CH_2                                                             \
    0x40000000UL /* Marker of internal channel for other ADC instances, in case of different ADC \
                    internal channels mapped on same channel number on different ADC instances */
#define ADC_CHANNEL_DIFFERENCIATION_TEMPSENSOR_VBAT                                               \
    0x10000000U /* Dummy bit for driver internal usage, not used in ADC channel setting registers \
                   CR1 or SQRx */
...
#define VREFINT_CAL_ADDR                                                                           \
    ((uint16_t*)(0x1FFF7A2AU)) /* Internal voltage reference, address of parameter VREFINT_CAL:    \
                                  VrefInt ADC raw data acquired at temperature 30 DegC (tolerance: \
                                  +-5 DegC), Vref+ = 3.3 V (tolerance: +-10 mV). */
#define VREFINT_CAL_VREF                                                                      \
    (3300UL) /* Analog voltage reference (Vref+) value with which temperature sensor has been \
...
#define TEMPSENSOR_CAL1_ADDR                                                                       \
    ((uint16_t*)(0x1FFF7A2CU)) /* Internal temperature sensor, address of parameter TS_CAL1: On    \
                                  STM32F4, temperature sensor ADC raw data acquired at temperature \
                                  30 DegC (tolerance: +-5 DegC), Vref+ = 3.3 V (tolerance: +-10    \
                                  mV). */
#define TEMPSENSOR_CAL2_ADDR                                                                       \
    ((uint16_t*)(0x1FFF7A2EU)) /* Internal temperature sensor, address of parameter TS_CAL2: On    \
                                  STM32F4, temperature sensor ADC raw data acquired at temperature \
                                  110 DegC (tolerance: +-5 DegC), Vref+ = 3.3 V (tolerance: +-10   \
                                  mV). */
#define TEMPSENSOR_CAL1_TEMP                                                                       \
    ((int32_t)30) /* Internal temperature sensor, temperature at which temperature sensor has been \
                     calibrated in production for data into TEMPSENSOR_CAL1_ADDR (tolerance: +-5   \
                     DegC) (unit: DegC). */
#define TEMPSENSOR_CAL2_TEMP                                                                       \
    ((int32_t)110) /* Internal temperature sensor, temperature at which temperature sensor has     \
                      been calibrated in production for data into TEMPSENSOR_CAL2_ADDR (tolerance: \
                      +-5 DegC) (unit: DegC). */
#define TEMPSENSOR_CAL_VREFANALOG                                                               \
    (3300UL) /* Analog voltage reference (Vref+) voltage with which temperature sensor has been \
                calibrated in production (+-10 mV) (unit: mV). */
...
typedef struct {
    ...
    uint32_t Multimode; /*!< Set ADC multimode configuration to operate in independent mode or
                           multimode (for devices with several ADC instances). This parameter can be
                           a value of @ref ADC_LL_EC_MULTI_MODE
...
typedef struct {
    ...
    uint32_t SequencersScanMode; /*!< Set ADC scan selection.
                                      ...
                                      This feature can be modified afterwards using unitary function
                                    @ref LL_ADC_SetSequencersScanMode(). */
...
typedef struct {
    uint32_t TriggerSource; /*!< Set ADC group regular conversion trigger source: internal (SW
                               start) or from external IP (timer event, external interrupt line).
                                 ...
                                 @note On this STM32 series, setting of external trigger edge is
                               performed using function @ref LL_ADC_REG_StartConversionExtTrig().
...
typedef struct {
    uint32_t TriggerSource; /*!< Set ADC group injected conversion trigger source: internal (SW
                               start) or from external IP (timer event, external interrupt line).
                                 ...
                                 @note On this STM32 series, setting of external trigger edge is
                               performed using function @ref LL_ADC_INJ_StartConversionExtTrig().
...
#define LL_ADC_FLAG_EOCS                                                                         \
    ADC_SR_EOC /*!< ADC flag ADC group regular end of unitary conversion or sequence conversions \
...
#define LL_ADC_FLAG_JEOS                                                                          \
    ADC_SR_JEOC /*!< ADC flag ADC group injected end of sequence conversions (Note: on this STM32 \
                   series, there is no flag ADC group injected end of unitary conversion. Flag    \
...
#define LL_ADC_FLAG_EOCS_MST                                                                   \
    ADC_CSR_EOC1 /*!< ADC flag ADC multimode master group regular end of unitary conversion or \
                    sequence conversions (to configure flag of end of conversion, use function \
                    @ref LL_ADC_REG_SetFlagEndOfConversion() ) */
#define LL_ADC_FLAG_EOCS_SLV1                                                                   \
    ADC_CSR_EOC2 /*!< ADC flag ADC multimode slave 1 group regular end of unitary conversion or \
                    sequence conversions (to configure flag of end of conversion, use function  \
                    @ref LL_ADC_REG_SetFlagEndOfConversion() ) */
#define LL_ADC_FLAG_EOCS_SLV2                                                                   \
    ADC_CSR_EOC3 /*!< ADC flag ADC multimode slave 2 group regular end of unitary conversion or \
                    sequence conversions (to configure flag of end of conversion, use function  \
                    @ref LL_ADC_REG_SetFlagEndOfConversion() ) */
...
#define LL_ADC_FLAG_JEOS_MST                                                                     \
    ADC_CSR_JEOC1 /*!< ADC flag ADC multimode master group injected end of sequence conversions  \
                     (Note: on this STM32 series, there is no flag ADC group injected end of     \
                     unitary conversion. Flag noted as "JEOC" is corresponding to flag "JEOS" in \
                     other STM32 families) */
#define LL_ADC_FLAG_JEOS_SLV1                                                                    \
    ADC_CSR_JEOC2 /*!< ADC flag ADC multimode slave 1 group injected end of sequence conversions \
                     (Note: on this STM32 series, there is no flag ADC group injected end of     \
                     unitary conversion. Flag noted as "JEOC" is corresponding to flag "JEOS" in \
                     other STM32 families) */
#define LL_ADC_FLAG_JEOS_SLV2                                                                    \
    ADC_CSR_JEOC3 /*!< ADC flag ADC multimode slave 2 group injected end of sequence conversions \
                     (Note: on this STM32 series, there is no flag ADC group injected end of     \
                     unitary conversion. Flag noted as "JEOC" is corresponding to flag "JEOS" in \
                     other STM32 families) */
...
#define LL_ADC_IT_EOCS                                                                          \
    ADC_CR1_EOCIE /*!< ADC interruption ADC group regular end of unitary conversion or sequence \
...
#define LL_ADC_IT_JEOS                                                                            \
    ADC_CR1_JEOCIE /*!< ADC interruption ADC group injected end of sequence conversions (Note: on \
                      this STM32 series, there is no flag ADC group injected end of unitary       \
                      conversion. Flag noted as "JEOC" is corresponding to flag "JEOS" in other   \
                      STM32 families) */
...
#define LL_ADC_DMA_REG_REGULAR_DATA                                                              \
    0x00000000UL /* ADC group regular conversion data register (corresponding to register DR) to \
                    be used with ADC configured in independent mode. Without DMA transfer,       \
                    register accessed by LL function @ref LL_ADC_REG_ReadConversionData32() and  \
                    other functions @ref LL_ADC_REG_ReadConversionDatax() */
#if defined(ADC_MULTIMODE_SUPPORT)
#define LL_ADC_DMA_REG_REGULAR_DATA_MULTI                                                          \
    0x00000001UL /* ADC group regular conversion data register (corresponding to register CDR) to  \
                    be used with ADC configured in multimode (available on STM32 devices with      \
                    several ADC instances). Without DMA transfer, register accessed by LL function \
...
#define LL_ADC_CLOCK_SYNC_PCLK_DIV4                                                              \
    (ADC_CCR_ADCPRE_0) /*!< ADC synchronous clock derived from AHB clock with prescaler division \
                          by 4 */
#define LL_ADC_CLOCK_SYNC_PCLK_DIV6                                                              \
    (ADC_CCR_ADCPRE_1) /*!< ADC synchronous clock derived from AHB clock with prescaler division \
                          by 6 */
#define LL_ADC_CLOCK_SYNC_PCLK_DIV8                                                              \
    (ADC_CCR_ADCPRE_1 | ADC_CCR_ADCPRE_0) /*!< ADC synchronous clock derived from AHB clock with \
...
#define LL_ADC_DATA_ALIGN_RIGHT                                                                    \
    0x00000000UL /*!< ADC conversion data alignment: right aligned (alignment on data register LSB \
                    bit 0)*/
#define LL_ADC_DATA_ALIGN_LEFT                                                                   \
    (ADC_CR2_ALIGN) /*!< ADC conversion data alignment: left aligned (alignment on data register \
                       MSB bit 15)*/
/**
 * @}
 */
...
#define LL_ADC_SEQ_SCAN_DISABLE                                                                   \
    0x00000000UL /*!< ADC conversion is performed in unitary conversion mode (one channel         \
                    converted, that defined in rank 1). Configuration of both groups regular and  \
                    injected sequencers (sequence length, ...) is discarded: equivalent to length \
                    of 1 rank.*/
#define LL_ADC_SEQ_SCAN_ENABLE                                                                   \
    (ADC_CR1_SCAN) /*!< ADC conversions are performed in sequence conversions mode, according to \
                      configuration of both groups regular and injected sequencers (sequence     \
                      length, ...). */
/**
 * @}
 */
...
#define LL_ADC_CHANNEL_VREFINT                                                                    \
    (LL_ADC_CHANNEL_17 |                                                                          \
     ADC_CHANNEL_ID_INTERNAL_CH) /*!< ADC internal channel connected to VrefInt: Internal voltage \
                                    reference. On STM32F4, ADC channel available only on ADC      \
                                    instance: ADC1. */
#define LL_ADC_CHANNEL_VBAT                                                                    \
    (LL_ADC_CHANNEL_18 |                                                                       \
     ADC_CHANNEL_ID_INTERNAL_CH) /*!< ADC internal channel connected to Vbat/3: Vbat voltage   \
                                    through a divider ladder of factor 1/3 to have Vbat always \
                                    below Vdda. On STM32F4, ADC channel available only on ADC  \
                                    instance: ADC1. */
...
#define LL_ADC_CHANNEL_TEMPSENSOR                                                              \
    (LL_ADC_CHANNEL_16 |                                                                       \
     ADC_CHANNEL_ID_INTERNAL_CH) /*!< ADC internal channel connected to Temperature sensor. On \
...
#define LL_ADC_CHANNEL_TEMPSENSOR                                                                \
    (LL_ADC_CHANNEL_18 | ADC_CHANNEL_ID_INTERNAL_CH |                                            \
     ADC_CHANNEL_DIFFERENCIATION_TEMPSENSOR_VBAT) /*!< ADC internal channel connected to         \
                                                     Temperature sensor. On STM32F4, ADC channel \
                                                     available only on ADC instance: ADC1. This  \
                                                     internal channel is shared between          \
                                                     temperature sensor and Vbat, only 1         \
                                                     measurement path must be enabled. */
#endif /* STM32F411xE || STM32F412Cx || STM32F412Rx || STM32F412Vx || STM32F412Zx || STM32F413xx \
          || STM32F423xx || STM32F427xx || STM32F437xx || STM32F429xx || STM32F439xx ||          \
...
#define LL_ADC_REG_TRIG_EXT_RISINGFALLING                                                         \
    (ADC_CR2_EXTEN_1 | ADC_CR2_EXTEN_0) /*!< ADC group regular conversion trigger polarity set to \
...
#define LL_ADC_REG_DMA_TRANSFER_LIMITED                                                           \
    (ADC_CR2_DMA) /*!< ADC conversion data are transferred by DMA, in limited mode (one shot      \
                     mode): DMA transfer requests are stopped when number of DMA data transfers   \
                     (number of ADC conversions) is reached. This ADC mode is intended to be used \
                     with DMA mode non-circular. */
#define LL_ADC_REG_DMA_TRANSFER_UNLIMITED                                                          \
    (ADC_CR2_DDS | ADC_CR2_DMA) /*!< ADC conversion data are transferred by DMA, in unlimited      \
...
/** @defgroup ADC_LL_EC_REG_FLAG_EOC_SELECTION ADC group regular - Flag EOC selection (unitary or
...
#define LL_ADC_REG_SEQ_SCAN_DISABLE                                                               \
    0x00000000UL /*!< ADC group regular sequencer disable (equivalent to sequencer of 1 rank: ADC \
...
#define LL_ADC_REG_SEQ_SCAN_ENABLE_10RANKS \
    (ADC_SQR1_L_3 |                        \
     ADC_SQR1_L_0) /*!< ADC group regular sequencer enable with 10 ranks in the sequence */
#define LL_ADC_REG_SEQ_SCAN_ENABLE_11RANKS \
    (ADC_SQR1_L_3 |                        \
     ADC_SQR1_L_1) /*!< ADC group regular sequencer enable with 11 ranks in the sequence */
...
#define LL_ADC_REG_SEQ_SCAN_ENABLE_13RANKS \
    (ADC_SQR1_L_3 |                        \
     ADC_SQR1_L_2) /*!< ADC group regular sequencer enable with 13 ranks in the sequence */
...
#define LL_ADC_INJ_TRIG_EXT_TIM1_CH4                                                             \
    (ADC_INJ_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group injected conversion trigger from external IP: \
                                       TIM1 channel 4 event (capture compare: input capture or   \
                                       output capture). Trigger edge set to rising edge (default \
                                       setting). */
#define LL_ADC_INJ_TRIG_EXT_TIM1_TRGO                                                            \
    (ADC_CR2_JEXTSEL_0 |                                                                         \
     ADC_INJ_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group injected conversion trigger from external IP: \
                                       TIM1 TRGO. Trigger edge set to rising edge (default       \
                                       setting). */
#define LL_ADC_INJ_TRIG_EXT_TIM2_CH1                                                             \
    (ADC_CR2_JEXTSEL_1 |                                                                         \
     ADC_INJ_TRIG_EXT_EDGE_DEFAULT) /*!< ADC group injected conversion trigger from external IP: \
                                       TIM2 channel 1 event (capture compare: input capture or   \
                                       output capture). Trigger edge set to rising edge (default \
                                       setting). */
...
#define LL_ADC_INJ_TRIG_EXT_RISINGFALLING                                                         \
    (ADC_CR2_JEXTEN_1 | ADC_CR2_JEXTEN_0) /*!< ADC group injected conversion trigger polarity set \
...
#define LL_ADC_INJ_TRIG_INDEPENDENT                                                               \
    0x00000000UL /*!< ADC group injected conversion trigger independent. Setting mandatory if ADC \
                    group injected injected trigger source is set to an external trigger. */
#define LL_ADC_INJ_TRIG_FROM_GRP_REGULAR                                                           \
    (ADC_CR1_JAUTO) /*!< ADC group injected conversion trigger from ADC group regular. Setting     \
                       compliant only with group injected trigger source set to SW start, without  \
                       any further action on  ADC group injected conversion start or stop: in this \
...
#define LL_ADC_INJ_SEQ_SCAN_DISABLE                                                                \
    0x00000000UL /*!< ADC group injected sequencer disable (equivalent to sequencer of 1 rank: ADC \
                    conversion on only 1 channel) */
...
#define LL_ADC_INJ_SEQ_SCAN_ENABLE_4RANKS \
    (ADC_JSQR_JL_1 |                      \
     ADC_JSQR_JL_0) /*!< ADC group injected sequencer enable with 4 ranks in the sequence */
/**
 * @}
 */
...
#define LL_ADC_AWD_ALL_CHANNELS_REG                                                         \
    (ADC_CR1_AWDEN) /*!< ADC analog watchdog monitoring of all channels, converted by group \
                       regular only */
#define LL_ADC_AWD_ALL_CHANNELS_INJ                                                          \
    (ADC_CR1_JAWDEN) /*!< ADC analog watchdog monitoring of all channels, converted by group \
                        injected only */
#define LL_ADC_AWD_ALL_CHANNELS_REG_INJ                                                   \
    (ADC_CR1_JAWDEN | ADC_CR1_AWDEN) /*!< ADC analog watchdog monitoring of all channels, \
...
#define LL_ADC_AWD_CHANNEL_8_REG_INJ                                                          \
    ((LL_ADC_CHANNEL_8 & ADC_CHANNEL_ID_MASK) | ADC_CR1_JAWDEN | ADC_CR1_AWDEN |              \
     ADC_CR1_AWDSGL) /*!< ADC analog watchdog monitoring of ADC external channel (channel     \
                        connected to GPIO pin) ADCx_IN8, converted by either group regular or \
                        injected */
...
#define LL_ADC_AWD_CH_TEMPSENSOR_REG_INJ                                                      \
    ((LL_ADC_CHANNEL_TEMPSENSOR & ADC_CHANNEL_ID_MASK) | ADC_CR1_JAWDEN | ADC_CR1_AWDEN |     \
     ADC_CR1_AWDSGL) /*!< ADC analog watchdog monitoring of ADC internal channel connected to \
                        Temperature sensor, converted by either group regular or injected */
#endif /* STM32F405xx || STM32F415xx || STM32F407xx || STM32F417xx || STM32F401xC || STM32F401xE \
          || STM32F410xx */
#if defined(STM32F411xE) || defined(STM32F412Cx) || defined(STM32F412Rx) || \
    defined(STM32F412Vx) || defined(STM32F412Zx) || defined(STM32F413xx) || \
...
#define LL_ADC_AWD_CH_TEMPSENSOR_REG                                                               \
    ((LL_ADC_CHANNEL_TEMPSENSOR & ADC_CHANNEL_ID_MASK) | ADC_CR1_AWDEN |                           \
     ADC_CR1_AWDSGL) /*!< ADC analog watchdog monitoring of ADC internal channel connected to      \
                        Temperature sensor, converted by group regular only. This internal channel \
                        is shared between temperature sensor and Vbat, only 1 measurement path     \
                        must be enabled. */
#define LL_ADC_AWD_CH_TEMPSENSOR_INJ                                                              \
    ((LL_ADC_CHANNEL_TEMPSENSOR & ADC_CHANNEL_ID_MASK) | ADC_CR1_JAWDEN |                         \
     ADC_CR1_AWDSGL) /*!< ADC analog watchdog monitoring of ADC internal channel connected to     \
                        Temperature sensor, converted by group injected only. This internal       \
                        channel is shared between temperature sensor and Vbat, only 1 measurement \
                        path must be enabled. */
#define LL_ADC_AWD_CH_TEMPSENSOR_REG_INJ                                                        \
    ((LL_ADC_CHANNEL_TEMPSENSOR & ADC_CHANNEL_ID_MASK) | ADC_CR1_JAWDEN | ADC_CR1_AWDEN |       \
     ADC_CR1_AWDSGL) /*!< ADC analog watchdog monitoring of ADC internal channel connected to   \
                        Temperature sensor, converted by either group regular or injected. This \
                        internal channel is shared between temperature sensor and Vbat, only 1  \
                        measurement path must be enabled. */
#endif /* STM32F411xE || STM32F412Cx || STM32F412Rx || STM32F412Vx || STM32F412Zx || STM32F413xx \
          || STM32F423xx || STM32F427xx || STM32F437xx || STM32F429xx || STM32F439xx ||          \
          STM32F446xx || STM32F469xx || STM32F479xx */
...
#define LL_ADC_MULTI_DUAL_INJ_ALTERN                                                           \
    (ADC_CCR_MULTI_3 |                                                                         \
     ADC_CCR_MULTI_0) /*!< ADC dual mode enabled: group injected alternate trigger. Works only \
                         with external triggers (not internal SW start) */
#define LL_ADC_MULTI_DUAL_REG_SIM_INJ_SIM                                                     \
    (ADC_CCR_MULTI_0) /*!< ADC dual mode enabled: Combined group regular simultaneous + group \
                         injected simultaneous */
#define LL_ADC_MULTI_DUAL_REG_SIM_INJ_ALT                                                     \
    (ADC_CCR_MULTI_1) /*!< ADC dual mode enabled: Combined group regular simultaneous + group \
                         injected alternate trigger */
#define LL_ADC_MULTI_DUAL_REG_INT_INJ_SIM                                                  \
    (ADC_CCR_MULTI_1 | ADC_CCR_MULTI_0) /*!< ADC dual mode enabled: Combined group regular \
                                           interleaved + group injected simultaneous */
#if defined(ADC3)
#define LL_ADC_MULTI_TRIPLE_REG_SIM_INJ_SIM                                                  \
    (ADC_CCR_MULTI_4 | ADC_CCR_MULTI_0) /*!< ADC triple mode enabled: Combined group regular \
                                           simultaneous + group injected simultaneous */
#define LL_ADC_MULTI_TRIPLE_REG_SIM_INJ_ALT                                                  \
    (ADC_CCR_MULTI_4 | ADC_CCR_MULTI_1) /*!< ADC triple mode enabled: Combined group regular \
...
#define LL_ADC_MULTI_TRIPLE_INJ_ALTERN                                                           \
    (ADC_CCR_MULTI_4 |                                                                           \
     ADC_CCR_MULTI_0) /*!< ADC triple mode enabled: group injected alternate trigger. Works only \
                         with external triggers (not internal SW start) */
#endif
/**
 * @}
...
#define LL_ADC_MULTI_REG_DMA_EACH_ADC                                                          \
    0x00000000UL /*!< ADC multimode group regular conversions are transferred by DMA: each ADC \
                    uses its own DMA channel, with its individual DMA transfer settings */
#define LL_ADC_MULTI_REG_DMA_LIMIT_1                                                               \
    (ADC_CCR_DMA_0) /*!< ADC multimode group regular conversions are transferred by DMA, one DMA   \
                       channel for all ADC instances (DMA of ADC master), in limited mode (one     \
                       shot mode): DMA transfer requests are stopped when number of DMA data       \
                       transfers (number of ADC conversions) is reached. This ADC mode is intended \
                       to be used with DMA mode non-circular. Setting of DMA mode 1: 2 or 3 (dual  \
                       or triple mode) half-words one by one, ADC1 then ADC2 then ADC3. */
#define LL_ADC_MULTI_REG_DMA_LIMIT_2                                                               \
    (ADC_CCR_DMA_1) /*!< ADC multimode group regular conversions are transferred by DMA, one DMA   \
                       channel for all ADC instances (DMA of ADC master), in limited mode (one     \
                       shot mode): DMA transfer requests are stopped when number of DMA data       \
                       transfers (number of ADC conversions) is reached. This ADC mode is intended \
                       to be used with DMA mode non-circular. Setting of DMA mode 2: 2 or 3 (dual  \
                       or triple mode) half-words one by one, ADC2&1 then ADC1&3 then ADC3&2. */
#define LL_ADC_MULTI_REG_DMA_LIMIT_3                                                               \
    (ADC_CCR_DMA_1 |                                                                               \
     ADC_CCR_DMA_0) /*!< ADC multimode group regular conversions are transferred by DMA, one DMA   \
                       ...
                       shot mode): DMA transfer requests are stopped when number of DMA data       \
                       transfers (number of ADC conversions) is reached. This ADC mode is intended \
                       to be used with DMA mode non-circular. Setting of DMA mode 3: 2 or 3 (dual  \
                       or triple mode) bytes one by one, ADC2&1 then ADC1&3 then ADC3&2. */
#define LL_ADC_MULTI_REG_DMA_UNLMT_1                                                             \
    (ADC_CCR_DDS |                                                                               \
     ADC_CCR_DMA_0) /*!< ADC multimode group regular conversions are transferred by DMA, one DMA \
                       channel for all ADC instances (DMA of ADC master), in unlimited mode: DMA \
                       transfer requests are unlimited, whatever number of DMA data transferred  \
                       (number of ADC conversions) is reached. This ADC mode is intended to be   \
                       used with DMA mode non-circular. Setting of DMA mode 1: 2 or 3 (dual or   \
...
#define LL_ADC_MULTI_REG_DMA_UNLMT_2                                                             \
    (ADC_CCR_DDS |                                                                               \
     ADC_CCR_DMA_1) /*!< ADC multimode group regular conversions are transferred by DMA, one DMA \
                       ...
                       transfer requests are unlimited, whatever number of DMA data transferred  \
                       (number of ADC conversions) is reached. This ADC mode is intended to be   \
                       used with DMA mode non-circular. Setting of DMA mode 2: 2 or 3 (dual or   \
                       triple mode) half-words by pairs, ADC2&1 then ADC1&3 then ADC3&2. */
#define LL_ADC_MULTI_REG_DMA_UNLMT_3                                                             \
    (ADC_CCR_DDS | ADC_CCR_DMA_1 |                                                               \
     ADC_CCR_DMA_0) /*!< ADC multimode group regular conversions are transferred by DMA, one DMA \
                       ...
                       transfer requests are unlimited, whatever number of DMA data transferred  \
                       (number of ADC conversions) is reached. This ADC mode is intended to be   \
                       used with DMA mode non-circular. Setting of DMA mode 3: 2 or 3 (dual or   \
 ...
 *         (2) On devices STM32F42x and STM32F43x, limitation: this internal channel is shared
 ...
 *         (2) On devices STM32F42x and STM32F43x, limitation: this internal channel is shared
 * between temperature sensor and Vbat, only 1 measurement path must be enabled.\n (1) For ADC
 * channel read back from ADC register, comparison with internal channel parameter to be done using
 * helper macro @ref __LL_ADC_CHANNEL_INTERNAL_TO_EXTERNAL().
 */
#define __LL_ADC_DECIMAL_NB_TO_CHANNEL(__DECIMAL_NB__)                                  \
    (((__DECIMAL_NB__) <= 9UL)                                                          \
         ? (((__DECIMAL_NB__) << ADC_CHANNEL_ID_NUMBER_BITOFFSET_POS) |                 \
            (ADC_SMPR2_REGOFFSET |                                                      \
             (((uint32_t)(3UL * (__DECIMAL_NB__))) << ADC_CHANNEL_SMPx_BITOFFSET_POS))) \
         : (((__DECIMAL_NB__) << ADC_CHANNEL_ID_NUMBER_BITOFFSET_POS) |                 \
            (ADC_SMPR1_REGOFFSET |                                                      \
 ...
 * value (unit uV/DegCelsius). On STM32F4, refer to device datasheet parameter "Avg_Slope".
 * @param  __TEMPSENSOR_TYP_CALX_V__     Device datasheet data Temperature sensor voltage typical
 * value (at temperature and Vref+ defined in parameters below) (unit mV). On STM32F4, refer to
 * device datasheet parameter "V25".
...
__STATIC_INLINE uint32_t LL_ADC_REG_GetTriggerSource(const ADC_TypeDef* ADCx) {
    ...
    uint32_t ShiftExten =
        ((TriggerSource & ADC_CR2_EXTEN) >> (ADC_REG_TRIG_EXTEN_BITOFFSET_POS - 2UL));
...
__STATIC_INLINE uint32_t LL_ADC_INJ_GetTriggerSource(const ADC_TypeDef* ADCx) {
    ...
    uint32_t ShiftExten =
        ((TriggerSource & ADC_CR2_JEXTEN) >> (ADC_INJ_TRIG_EXTEN_BITOFFSET_POS - 2UL));
...
__STATIC_INLINE void LL_ADC_INJ_SetSequencerRanks(ADC_TypeDef* ADCx, uint32_t Rank,
    ...
    MODIFY_REG(
        ADCx->JSQR, ADC_CHANNEL_ID_NUMBER_MASK << (5UL * (uint8_t)(((Rank) + 3UL) - (tmpreg1))),
